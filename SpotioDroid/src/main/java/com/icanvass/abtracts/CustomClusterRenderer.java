...
/**
 * Created by dev2 on 08.09.14.
 */
//purpose to fix https://github.com/googlemaps/android-maps-utils/issues/56 issue
...
//    private static final int MIN_CLUSTER_SIZE = 5;
    ...
    private class RenderTask implements Runnable {
        ...
        @SuppressLint("NewApi")
        public void run() {
            if (clusters.equals(CustomClusterRenderer.this.mClusters)) {
                mCallback.run();
                return;
            }
            ...
            if (CustomClusterRenderer.this.mClusters != null && SHOULD_ANIMATE) {
                existingClustersOnScreen = new ArrayList<Point>();
                for (Cluster<T> c : CustomClusterRenderer.this.mClusters) {
            ...
            mMarkers = newMarkers;
            CustomClusterRenderer.this.mClusters = clusters;
            mZoom = zoom;
...
//        Log.e(TAG,"onClustersChanged=>"+clusters.size());

        for (Cluster<T> c : clusters) {
            Log.e(TAG,"onClustersChanged=>"+c.getSize());
...
//            descriptor = BitmapDescriptorFactory.fromBitmap(mIconGenerator.makeIcon(getClusterText(bucket)));
            Log.e("MapFragment","CustomClusterRender,cluster size=>"+cluster.getSize());
            shouldRender = true;
            descriptor = BitmapDescriptorFactory.fromBitmap(mIconGenerator.makeIcon(String.valueOf(cluster.getSize())));
//            descriptor = BitmapDescriptorFactory.fromBitmap(mIconGenerator.makeIcon(cluster.getSize());
        ...
        //        Log.e("MapFragment","===============================================");
//        if (shouldRender) {
//            Log.e("MapFragment","perform,rendering new=>"+shouldRenderAsCluster(cluster));
//            shouldRender = false;
...
//        Log.e("MapFragment","onClusterItemRendered");
